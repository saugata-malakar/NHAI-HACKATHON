...
export default function EnrollScreen() {
  ...
  React.useEffect(() => {
    try {
      ...
      console.warn('[EnrollScreen] Screenshot prevention not available:', err);
    }
    return () => {
      try {
  ...
  const onFrame = Worklets.createRunOnJS(async (
    ...
    try {
      ...
      try {
        ...
        EdgeLogger.error('EnrollScreen', `BlazeFace model execution failed: ${err?.message || err}`);
        throw err;
      }
      ...
      try {
        ...
        EdgeLogger.error('EnrollScreen', `FaceMesh landmark model execution failed: ${err?.message || err}`);
        throw err;
      }
      if (!landmarks) return;
      ...
      if (stepRef.current === 'capturing') {
        ...
        try {
          ...
          EdgeLogger.error('EnrollScreen', `MiniFAS net passive liveness model execution failed: ${err?.message || err}`);
          throw err;
        }
        ...
        try {
          ...
          EdgeLogger.error('EnrollScreen', `MobileFaceNet embedding extraction model execution failed: ${err?.message || err}`);
          throw err;
        }
      ...
      EdgeLogger.error('EnrollScreen', `Enrollment frame processor error: ${errMsg}`);
      console.error('[EnrollScreen] Frame processing error:', err);
    }
  });
  ...
  const saveEnrollment = async () => {
    try {
      ...
      // Incrementally update LSHIndex prior to in-place memory scrubbing inside enrollFace
      LSHIndex.addEntry({
        userId: userId.trim(),
